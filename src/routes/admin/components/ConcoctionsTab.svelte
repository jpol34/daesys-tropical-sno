<script lang="ts">
	...
	async function addConcoction() {
		...
		const ingredients = newConcoctionIngredients
			.split(',')
			.map((i) => i.trim())
			.filter(Boolean);
...
<div class="management-section">
	<div class="add-form add-form-vertical">
		<input
			type="text"
			class="form-input"
			placeholder="Concoction name..."
			bind:value={newConcoctionName}
		/>
		<input
			type="text"
			class="form-input"
			placeholder="Ingredients (comma separated)..."
			bind:value={newConcoctionIngredients}
		/>
		<button
			class="btn btn-primary"
			onclick={addConcoction}
			disabled={isUpdating || !newConcoctionName.trim() || !newConcoctionIngredients.trim()}
		>
			+ Add Concoction
		</button>
	</div>
			...
			<div class="item-row concoction-row" class:inactive={!concoction.active}>
				{#if editingConcoction?.id === concoction.id}
					<div class="edit-concoction">
						<input
							...
							class="form-input"
							placeholder="Name"
							bind:value={editingConcoction.name}
						/>
						<input
							type="text"
							class="form-input"
							placeholder="Ingredients (comma separated)"
							value={editingConcoction.ingredients.join(', ')}
							oninput={(e) =>
								(editingConcoction!.ingredients = (e.target as HTMLInputElement).value
									.split(',')
									.map((i) => i.trim()))}
						/>
						<div class="item-actions">
							<button
								...
								onclick={() => updateConcoction(editingConcoction!)}>Save</button
							>
							<button class="btn btn-small" onclick={() => (editingConcoction = null)}
								>Cancel</button
							>
						</div>
					</div>
				{:else}
					...
					<div class="item-actions">
						<button
							class="btn btn-small"
							class:active={concoction.active}
							onclick={() => toggleConcoctionActive(concoction)}
						>
							{concoction.active ? '✓ Active' : '✗ Hidden'}
						</button>
						<button class="btn btn-small" onclick={() => (editingConcoction = { ...concoction })}
							>Edit</button
						>
						<button
							class="btn btn-small btn-delete-small"
							onclick={() => deleteConcoction(concoction.id)}>×</button
						>
					</div>
				{/if}
			</div>
	...
	<p class="management-note">
		💡 Hidden concoctions won't appear on the website but are kept for future use.
	</p>
</div>
